...
public class CartShareOrdController {
    ...
    public ResponseEntity<SuccessResponse<String>> saveCartShareOrd(@PathVariable Long cartShareId) {

        cartShareOrdService.saveCartShareOrd(cartShareId);

        return SuccessResponse.OK;
    }

    @Operation(summary = "공유장바구니 리스트 조회", responses = {
            @ApiResponse(responseCode = "200", description = "공유장비구니 리스트 조회 성공")
    })
    @GetMapping(value = "/cart-share/{cartShareId}/cart-share-ord", params = "mbrIdList")
    public ResponseEntity<SuccessResponse<CartShareOrdFindListResponse>> findCartShareOrdList(
        ...
        return SuccessResponse.success(SuccessCode.FIND_CART_SHARE_ORD_SUCCESS,
                cartShareOrdService.findCartShareOrderList(cartShareId));
    }
